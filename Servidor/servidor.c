...
main()
{
	...
	do
	{
		...
		do
		{
			...
			switch (estado)
			{
				...
				case S_PASS: /******************************************************/
					...
					if ( strcmp(cmd,PW)==0 ) // si comando recibido es password
					{
						...
						{
							sprintf_s (buffer_out, sizeof(buffer_out), "%s Fin de la conexión%s", OK,CRLF); //formato de fin de conexión
							fin_conexion=1;
						}
					else
					{
							sprintf_s (buffer_out, sizeof(buffer_out), "%s Comando incorrecto%s",ER,CRLF); //formato de fin de conxión
					}
				break;
			...
			//TODO (Correción de errores de envio)
			if (enviados==0){
				printf("SERVIDOR> CONEXION LIBERADA CORRECTAMENTE\r\n");
				return 0;
			}
